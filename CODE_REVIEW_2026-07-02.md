...
> **처리 결과 (by-design / won't-fix)**: 중복은 **의도된 설계**로 확인되어 dedupe 하지 않는다.
> 각 하니스(`<software>/agent-harness/`)는 **독립적으로 배포·설치 가능한 자체 완결 Python 패키지**이며,
> 저마다 `setup.py`를 가지고 `pip install -e .`(또는 `pip install git+https://...`)로 **단독 설치**된다
> (`CONTRIBUTING.md`의 "Development Setup" 및 Option 2 standalone repo 규정). `repl_skin.py`가 각 하니스의
> `utils/`에 **미변형 복사본**으로 존재해야 한다는 것은 계약으로 명문화되어 있다:
> `.github/PULL_REQUEST_TEMPLATE.md`("`repl_skin.py` in `utils/` is an unmodified copy from the plugin")와
> `CONTRIBUTING.md`(항목 6). 공용 `cli-anything-core`로 추출하면 단일 하니스만 설치했을 때 의존성이 깨져
> standalone 설치 모델이 무너진다. 따라서 파일 수·중복은 이식성(portability)을 위한 **구조적 트레이드오프**이며,
> 드리프트 억제는 dedupe가 아니라 PR 템플릿의 "미변형 복사본" 게이트로 관리한다. (근거: 하니스별 `setup.py` 58개,
> `repl_skin.py` 48개 중 39개 md5 동일 — 남은 변형은 별도 추적 대상.)
...
> **처리 진행 (burndown 착수)**: 동작 보존(behavior-preserving) 분할로 baseline을 **370건 → 360건**으로 축소했다.
> 깨끗한 함수/클래스 경계를 가진 `core/*.py` 10개를 추출 모듈 + 재노출(re-export) 패턴으로 분리했다(각 파일 ≤200줄,
> 원본은 공개 import 경로 유지). 분할 후 `--write-baseline`로 실제 축소분만 반영. 나머지 초대형 파일
> (`freecad_cli.py`, `sbox_cli.py`, `preview.py` 등)은 후속 배치에서 계속 축소한다.
