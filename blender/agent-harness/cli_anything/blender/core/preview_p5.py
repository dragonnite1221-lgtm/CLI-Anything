...
from .preview_p1 import (
    ...
    _now_iso,
    _pid_is_running,
    _project_file_fingerprint,
    _with_live_refs,
    _write_json,
)
from .preview_p2 import (
    _history_item,
    _load_existing_live_session,
...
def _publish_live_session(
    ...
    *,
    ...
    current_refresh_hint_ms = int(
        refresh_hint_ms or existing.get("refresh_hint_ms") or DEFAULT_REFRESH_HINT_MS
    )
    ...
    payload = {
        ...
        "poller": poller,
        "publish_command": f"cli-anything-blender{project_flag} preview live push --recipe {recipe}{root_flag}".strip(),
        ...
        "start_command": f"cli-anything-blender{project_flag} preview live start --recipe {recipe} --mode {current_live_mode} --source-poll-ms {current_source_poll_ms}{root_flag}".strip(),
        "monitor_command": f"cli-anything-blender preview live monitor --session-dir {session_dir}",
    }
