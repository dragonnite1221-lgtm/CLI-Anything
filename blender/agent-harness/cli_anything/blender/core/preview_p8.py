...
from .preview_p1 import _now_iso, _project_file_fingerprint
from .preview_p2 import _load_existing_live_session, _write_live_session_updates
from .preview_p6 import live_start
...
def poll_live_session_once(session_dir: str) -> Dict[str, Any]:
    ...
    base_updates: Dict[str, Any] = {
        "poller": {"pid": os.getpid(), "running": True, "last_heartbeat": now},
        "source_state": {"source_type": "project-file", "project_path": project_path},
    }
    ...
    try:
        ...
        live_payload = live_start(
            ...
            command=str(
                ...
                or f"cli-anything-blender preview live monitor --session-dir {session_path}"
            ),
            publish_reason="auto-poll",
        )
        render_time = _coupbase._COUP_GLOBALS["_now_iso"]()
        _write_live_session_updates(
            session_path,
            {
                "poller": {
        ...
        error_time = _coupbase._COUP_GLOBALS["_now_iso"]()
        _write_live_session_updates(
            session_path,
            {
                "poller": {
