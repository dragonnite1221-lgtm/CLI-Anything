...
def test_secure_install_uses_the_verified_runtime_installer(monkeypatch):
    monkeypatch.setattr(
        browser_cli.backend,
        "install_secure_runtime",
        lambda: {"installed": True, "version": "2.0.10"},
    )

    result = CliRunner().invoke(browser_cli.cli, ["--json", "secure", "install"])

    assert result.exit_code == 0
    assert '"installed": true' in result.output
    assert '"version": "2.0.10"' in result.output
