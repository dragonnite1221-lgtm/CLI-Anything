...
def test_pinned_connection_uses_async_resolver(monkeypatch):
    seen: list[tuple[str, int]] = []
    ...
    async def exercise():
        ...
        monkeypatch.setattr(socket, "getaddrinfo", lambda *_args, **_kwargs: pytest.fail("synchronous resolver used"))
        monkeypatch.setattr(proxy.asyncio, "open_connection", fake_open_connection)
        await proxy.open_pinned_connection("example.com", 443)
