...
DOMSHELL_NPM_SPEC = "@apireno/domshell@2.0.10"
...
def _terminate_process_group(pid: int) -> None:
    """Terminate an npx session including its detached Node child process."""
...
def _start_mcp_server(token: str, mcp_port: int, ws_port: int) -> subprocess.Popen[bytes]:
    ...
    if not executable:
        raise ManagedDOMShellError("npx is required for the pinned DOMShell MCP server")
...
def _wait_for_port(port: int) -> None:
    # The first pinned npx launch may need to populate an empty package cache.
    # Once present, this is normally immediate; the bounded longer wait avoids
    # terminating a legitimate first install halfway through dependency setup.
