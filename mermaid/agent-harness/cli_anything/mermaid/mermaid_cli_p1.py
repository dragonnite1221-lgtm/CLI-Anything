...
@click.option(
    "--project", "project_path", default=None, help="Open a Mermaid project file"
)
@click.option(
    "--dry-run",
    "dry_run",
...
def repl() -> None:
    ...
    while True:
        ...
        line = skin.get_input(
            prompt_session, project_name=project_name, modified=session.modified
        ).strip()
        if not line:
            continue
        ...
        try:
            ...
            if command == "new":
                ...
                emit(
                    project_mod.new_project(session, sample=sample),
                    "Created Mermaid project",
                )
            elif command == "open":
                emit(
                    project_mod.open_project(session, args[0]), "Opened Mermaid project"
                )
            elif command == "save":
                ...
                emit(
                    diagram_mod.set_diagram(session, " ".join(args)),
                    "Updated Mermaid source",
                )
            elif command == "render":
                fmt = args[1] if len(args) > 1 else "svg"
                emit(
                    export_mod.render(session, args[0], fmt=fmt, overwrite=True),
                    "Rendered output",
                )
            elif command == "share":
...
@click.option(
    "-o", "--output", "output_path", default=None, help="Optional file path to save"
)
