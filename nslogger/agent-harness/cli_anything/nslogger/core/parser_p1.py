...
def _part_data(
    raw: bytes, offset: int, part_type: int, *, implicit_int_sizes: bool
) -> tuple[bytes, int]:
    if implicit_int_sizes and part_type in (
        PART_TYPE_INT16,
        PART_TYPE_INT32,
        PART_TYPE_INT64,
    ):
        part_len = {PART_TYPE_INT16: 2, PART_TYPE_INT32: 4, PART_TYPE_INT64: 8}[
            part_type
        ]
...
def _parse_message_payload(
    ...
    *,
    ...
    for _ in range(part_count):
        ...
        offset += 2
        part_data, offset = _part_data(
            raw, offset, part_type, implicit_int_sizes=implicit_int_sizes
        )
        ...
        if part_key == PART_KEY_MESSAGE_TYPE:
            msg.message_type = (
                value if isinstance(value, int) else int.from_bytes(part_data, "big")
            )
            ...
            if isinstance(value, bytes) and part_type in (
                PART_TYPE_BINARY,
                PART_TYPE_IMAGE,
            ):
