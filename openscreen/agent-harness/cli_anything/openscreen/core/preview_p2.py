...
# fmt: off
from .preview_p1 import _attach_trajectory_ref, _metrics, _project_fingerprint, _trajectory_dir  # noqa: E402,E501
# fmt: on
...
def capture(
    ...
    *,
    ...
    trajectory_rel = os.path.relpath(
        Path(trajectory_dir) / "trajectory.json", Path(bundle_dir)
    )
    ...
    duration_s = float(
        clip_meta.get("duration", render_result.get("duration", 0.0)) or 0.0
    )
    ...
    source_video = session.data.get("media", {}).get(
        "screenVideoPath"
    ) or session.data.get("videoPath")
    manifest = finalize_bundle(
        ...
        generator={
            ...
            "harness_version": HARNESS_VERSION,
            "command": command
            or f"cli-anything-openscreen preview capture --recipe {recipe}",
        },
        status="partial" if warnings else "ok",
        warnings=warnings or None,
        context={
            "editor": {
                "aspectRatio": metrics["aspect_ratio"],
