...
class TestDoctor:
    ...
    def test_doctor_all_ok_json(
        self, mock_run: MagicMock, mock_which: MagicMock, runner: CliRunner
    ) -> None:
        mock_which.side_effect = lambda name: f"/usr/local/bin/{name}"
        mock_run.return_value = MagicMock(
            returncode=0, stdout="hevc_videotoolbox", stderr=""
        )
    ...
    def test_doctor_no_ffmpeg_json(
        self, _mock_which: MagicMock, runner: CliRunner
    ) -> None:
        result = runner.invoke(cli, ["doctor", "--json"])
        data = json.loads(result.output)
        assert data["ready"] is False
        ffmpeg_check = next(
            c for c in data["checks"] if c["check"] == "ffmpeg installed"
        )
...
class TestProbe:
    @patch("shutil.which", return_value=None)
    def test_probe_no_ffprobe(
        self, _mock_which: MagicMock, runner: CliRunner, tmp_path: Path
    ) -> None:
