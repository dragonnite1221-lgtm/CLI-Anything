...
def _find_compositing_transition(
    tractor: ET.Element, track_index: int
) -> Optional[ET.Element]:
...
def set_track_blend_mode(session: Session, track_index: int, blend_mode: str) -> dict:
...
def get_track_blend_mode(session: Session, track_index: int) -> dict:
    ...
    return {"track_index": track_index, "blend_mode": mode, "description": desc}


def set_track_opacity(session: Session, track_index: int, opacity: float) -> dict:
    ...
    for filt in playlist.findall("filter"):
        ...
        if (
            svc == "brightness"
            and mlt_xml.get_property(filt, "shotcut:filter") == "opacity"
        ):
            mlt_xml.set_property(filt, "alpha", str(opacity))
            return {
                "action": "set_track_opacity",
                "track_index": track_index,
                "opacity": opacity,
            }
    ...
    filt = mlt_xml.add_filter_to_element(
        ...
        properties={"alpha": str(opacity), "level": "1"},
    )
